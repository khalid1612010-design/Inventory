...
const translations = {
    en: {
        ...
        recordError: 'Could not save. Please check your connection and try again.',
        ...
        nameUpdated: 'Product name updated successfully!',
    ...
    ar: {
        ...
        recordError: 'تعذّر الحفظ. تأكد من الاتصال وحاول مرة أخرى.',
        ...
        newName: 'اسم المنتج الجديد',
        nameUpdated: 'تم تحديث اسم المنتج بنجاح!',
...
const EMPLOYEES = ['مي', 'سهيلة', 'رحمة'];

function buildEmployeeOptions(selected = '') {
    return `<option value="">${t('selectEmployee')}</option>` +
        EMPLOYEES.map(name => `<option value="${name}"${name === selected ? ' selected' : ''}>${name}</option>`).join('');
}
...
function formatCurrency(amount) {
    ...
    return new Intl.NumberFormat(AppState.currentLang === 'ar' ? 'ar-EG' : 'en-EG', {
            ...
            <form id="inventoryInForm" onsubmit="handleInventoryInSubmit(event)">
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select" id="inEmployeeName" required>
...
async function initInventoryInForm() {
    if (AppState.products.length === 0) {
        try { AppState.products = await ProductsAPI.getAll(); } catch (e) { /* silent */ }
    }
...
async function handleInventoryInSubmit(event) {
    ...
    try {
        ...
        // Show a friendly error message (with reason for diagnosis)
        showToast(t('recordError') + (err && err.message ? ' (' + err.message + ')' : ''), 'error');
    } finally {
        hideLoading();
    }
}
...
function renderProductsTable(products) {
    ...
    tbody.innerHTML = products.map(p => `
        <tr>
            ...
            <td id="qtyCell_${p.id}">${formatNumber(p.quantity)}</td>
            <td>${formatCurrency(p.unit_price)}</td>
            <td><strong>${formatCurrency(parseFloat(p.quantity || 0) * parseFloat(p.unit_price || 0))}</strong></td>
            <td>${escapeHtml(p.supplier_name || '-')}</td>
            <td>${formatDate(p.updated_at)}</td>
            <td style="display:flex;gap:0.4rem;flex-wrap:wrap;">
                <button class="btn btn-sm btn-outline" onclick="openEditName('${p.id}')" title="${t('editName')}">
                    🏷️ ${t('editName')}
                </button>
                <button class="btn btn-sm btn-outline" onclick="openEditQuantity('${p.id}')" title="${t('adjustQuantity')}">
                    ✏️ ${t('edit')}
                </button>
                <button class="btn btn-sm btn-danger" onclick="openDeleteProduct('${p.id}')" title="${t('delete')}">
                    🗑️ ${t('delete')}
                </button>
            </td>
        </tr>
...
// --- Edit quantity modal logic ---

function openEditQuantity(productId) {
    const product = AppState.products.find(p => p.id === productId);
    if (!product) return;

    const overlay = document.getElementById('editQtyModal');
    // Localize modal texts
    document.getElementById('editQtyModalTitle').textContent = t('adjustQuantity');
    document.getElementById('editQtyLabel').textContent = t('newQuantity');
    document.getElementById('editQtyCancelBtn').textContent = t('cancel');
    document.getElementById('editQtySaveBtn').textContent = t('save');
    document.getElementById('editQtyProductName').textContent = product.name;
    document.getElementById('editQtyInput').value = product.quantity;
    document.getElementById('editQtyForm').dataset.productId = productId;
    overlay.classList.add('active');
    setTimeout(() => document.getElementById('editQtyInput').focus(), 100);
}

function closeEditQuantity() {
    document.getElementById('editQtyModal').classList.remove('active');
}
...
async function saveEditQuantity(event) {
    ...
    try {
        showLoading();
        // Update the product quantity directly (absolute value)
...
// --- Edit product name modal logic ---

function openEditName(productId) {
    const product = AppState.products.find(p => p.id === productId);
    if (!product) return;

    document.getElementById('editNameTitle').textContent = t('editProduct');
    document.getElementById('editNameLabel').textContent = t('newName');
    document.getElementById('editNameCancelBtn').textContent = t('cancel');
    document.getElementById('editNameSaveBtn').textContent = t('save');
    document.getElementById('editNameInput').value = product.name;
    document.getElementById('editNameForm').dataset.productId = productId;
...
async function saveEditName(event) {
    ...
    if (!newName || newName === product.name) { closeEditName(); return; }

    try {
        showLoading();
        await ProductsAPI.updateById(productId, { name: newName });
        closeEditName();
...
function openDeleteProduct(productId) {
    ...
    const overlay = document.getElementById('deleteProductModal');
    document.getElementById('deleteProductTitle').textContent = t('delete');
    document.getElementById('deleteProductMsg').textContent = t('confirmDeleteProduct');
    document.getElementById('deleteProductName').textContent = product.name;
    document.getElementById('deleteProductCancelBtn').textContent = t('cancel');
    document.getElementById('deleteProductConfirmBtn').textContent = t('delete');
    document.getElementById('deleteProductConfirmBtn').dataset.productId = productId;
    overlay.classList.add('active');
}
...
async function confirmDeleteProduct() {
    const productId = document.getElementById('deleteProductConfirmBtn').dataset.productId;
    if (!productId) return;
