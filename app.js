 ...
 * business logic for the inventory management system.
 */
...
function formatDate(dateStr) {
    ...
    return d.toLocaleDateString(AppState.currentLang === 'ar' ? 'ar-SA' : 'en-US', {
        year: 'numeric',
        month: 'short',
        day: 'numeric'
    });
...
async function loadDashboardData() {
    try {
        ...
        const [products, inRecords, outRecords] = await Promise.all([
            ProductsAPI.getAll(),
            InventoryInAPI.getAll(),
            InventoryOutAPI.getAll()
        ]);
        ...
        const totalInOps = inRecords.length;
        const totalOutOps = outRecords.length;
        ...
        document.getElementById('statInOps').textContent = formatNumber(totalInOps);
        document.getElementById('statOutOps').textContent = formatNumber(totalOutOps);
        
        // Recent operations (last 10, combined)
        const allOps = [
            ...
            <form id="inventoryInForm" onsubmit="handleInventoryInSubmit(event)">
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-input" id="inProductName" required 
                               placeholder="${t('productName')}" list="productSuggestions">
                        <datalist id="productSuggestions"></datalist>
                    </div>
                    <div class="form-group">
                        <label class="form-label">${t('quantity')} <span class="required">*</span></label>
                        <input type="number" class="form-input" id="inQuantity" required 
                               min="0.01" step="0.01" placeholder="0" oninput="calcInTotalPrice()">
                    </div>
                    <div class="form-group">
                        <label class="form-label">${t('unitPrice')} <span class="required">*</span></label>
                        <input type="number" class="form-input" id="inUnitPrice" required 
                               min="0.01" step="0.01" placeholder="0.00" oninput="calcInTotalPrice()">
                    </div>
                    <div class="form-group">
                        <label class="form-label">${t('totalPrice')} <span class="required">*</span></label>
                        <input type="number" class="form-input calculated" id="inTotalPrice" required 
                               min="0.01" step="0.01" placeholder="0.00" oninput="calcInUnitPrice()">
...
async function initInventoryInForm() {
    // Ensure we have products loaded for suggestions
    if (AppState.products.length === 0) {
        try {
            AppState.products = await ProductsAPI.getAll();
        } catch (err) {
            console.warn('Could not load products for suggestions:', err.message);
        }
    }
...
function calcInTotalPrice() {
    const quantity = parseFloat(document.getElementById('inQuantity').value) || 0;
    const unitPrice = parseFloat(document.getElementById('inUnitPrice').value) || 0;
    if (quantity > 0 && unitPrice > 0) {
        document.getElementById('inTotalPrice').value = (quantity * unitPrice).toFixed(2);
    }
}

function calcInUnitPrice() {
    const quantity = parseFloat(document.getElementById('inQuantity').value) || 0;
    const totalPrice = parseFloat(document.getElementById('inTotalPrice').value) || 0;
    if (quantity > 0 && totalPrice > 0) {
        document.getElementById('inUnitPrice').value = (totalPrice / quantity).toFixed(2);
    }
}
...
async function handleInventoryInSubmit(event) {
    ...
    try {
        ...
        if (record.quantity <= 0) {
            showToast(t('error') + ': Quantity must be greater than 0', 'error');
            hideLoading();
            return;
        }
        
        // Save inventory in record
        const savedRecord = await InventoryInAPI.add(record);
        ...
        // Refresh product list for suggestions
        AppState.products = await ProductsAPI.getAll();
        updateProductSuggestions();
        ...
        <div class="form-card">
            ...
            <form id="inventoryOutForm" onsubmit="handleInventoryOutSubmit(event)">
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        <label class="form-label">${t('quantity')} <span class="required">*</span></label>
                        <input type="number" class="form-input" id="outQuantity" required 
                               min="0.01" step="0.01" placeholder="0" oninput="validateOutQuantity()">
                        <span class="form-hint" id="outQuantityHint"></span>
                    </div>
                    <div class="form-group">
                        <label class="form-label">${t('totalSellingPrice')} <span class="required">*</span></label>
                        <input type="number" class="form-input" id="outTotalPrice" required 
...
async function handleInventoryOutSubmit(event) {
    ...
    try {
        ...
        // Save inventory out record
        await InventoryOutAPI.add(record);
        
        // Decrease stock (negative delta)
...
function getAllOperations() {
    ...
    return allOps.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
}
...
function filterOperationsTable() {
    ...
    if (searchQuery) {
        ops = ops.filter(op => 
            op.product_name.toLowerCase().includes(searchQuery) ||
            (op.contactField && op.contactField.toLowerCase().includes(searchQuery))
        );
    }
    
    if (dateFilter) {
        ops = ops.filter(op => {
            const opDate = (op.dateField || '').split('T')[0];
    ...
    if (employeeFilter) {
        ops = ops.filter(op => 
            op.employee_name && op.employee_name.toLowerCase().includes(employeeFilter)
        );
    }
    
    if (typeFilter !== 'all') {
        ops = ops.filter(op => op.opType === typeFilter);
    }
...
async function downloadOperationPDF(id, opType) {
    try {
        ...
        if (opType === 'in') {
            ...
            if (record) {
                await PDFGenerator.generateInventoryInPDF(record, translations[AppState.currentLang]);
            }
        } else {
            record = await InventoryOutAPI.getById(id);
            if (record) {
                await PDFGenerator.generateInventoryOutPDF(record, translations[AppState.currentLang]);
            }
        }
        
        if (!record) {
            showToast(t('error') + ': Record not found', 'error');
        }
    } catch (err) {
...
// ============================================================
// PRODUCT SUGGESTIONS (for Inventory In form)
// ============================================================
