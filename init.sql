...
-- 4. CREATE INVENTORY RETURNS TABLE
CREATE TABLE IF NOT EXISTS public.inventory_returns (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    out_id TEXT DEFAULT '',
    product_name TEXT NOT NULL,
    quantity NUMERIC NOT NULL,
    return_type TEXT DEFAULT 'all',
    customer_name TEXT DEFAULT '',
    date DATE,
    employee_name TEXT DEFAULT '',
    notes TEXT DEFAULT '',
...
-- Inventory Returns: full access
CREATE POLICY "Allow all on inventory_returns" ON public.inventory_returns
    FOR ALL TO anon
    USING (true)
    WITH CHECK (true);
...
GRANT ALL ON public.inventory_returns TO anon;
