...
var ProductsAPI = window.ProductsAPI = {
    ...
    async updateProduct(oldName, id, newData) {
        try {
            ...
            if (id) {
                const { data, error } = await client.from('products').update(updatePayload).eq('id', id).select();
                if (!error && data && data.length > 0) updatedProduct = data[0];
            }
            if (!updatedProduct && oldName) {
                const { data, error } = await client.from('products').update(updatePayload).eq('name', oldName).select();
                if (!error && data && data.length > 0) updatedProduct = data[0];
            }
