    ...
    async updateProduct(oldName, id, newData) {
        try {
            ...
            if (id) {
                ...
                if (error && updatePayload.image_url !== undefined) {
                    delete updatePayload.image_url;
                    ({ data, error } = await client.from('products').update(updatePayload).eq('id', id).select());
                }
            ...
            if (!updatedProduct && oldName) {
                ...
                if (error && updatePayload.image_url !== undefined) {
                    delete updatePayload.image_url;
                    ({ data, error } = await client.from('products').update(updatePayload).eq('name', oldName).select());
                }
                if (!error && data && data.length > 0) updatedProduct = data[0];
            }
